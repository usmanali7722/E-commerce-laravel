...
function myOrders(){
    $userId=Session::get('user')['id'];
    $orders= $products=DB::table('orders')->join('table_products','orders.product_id','=','table_products.id')->where('orders.user_id',$userId)->sum('table_products.price');
       return view('ordernow',['orders'=>$orders]);
}
}